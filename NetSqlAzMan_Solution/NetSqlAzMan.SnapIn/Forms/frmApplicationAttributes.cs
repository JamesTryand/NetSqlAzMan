...
{
    public partial class frmApplicationAttributes : frmBase
    {
        ...
        [PreEmptive.Attributes.Feature("NetSqlAzMan MMC SnapIn: Application Attributes")]
        public frmApplicationAttributes()
        {
            InitializeComponent();
        ...
        private void frmApplicationAttributes_Load(object sender, EventArgs e)
        {
            this.DialogResult = DialogResult.None;
            this.refreshApplicationAttributes();
            this.modified = false;
        ...
        private ListViewItem CreateApplicationAttributeListViewItem(IAzManAttribute<IAzManApplication> Attribute)
        {
        ...
        private ListViewItem CreateApplicationAttributeListViewItem(KeyValuePair<string, string> keyValue)
        {
            ListViewItem lvi = new ListViewItem();
            lvi.Tag = keyValue;
            lvi.ImageIndex = 0;
        ...
        private void refreshApplicationAttributes()
        {
            this.HourGlass(true);
            this.lsvAttributes.Items.Clear();
            IAzManAttribute<IAzManApplication>[] applicationAttributes = this.application.GetAttributes();
            foreach (IAzManAttribute<IAzManApplication> applicationAttribute in applicationAttributes)
            {

                KeyValuePair<string, string> attrToUpdate = ((KeyValuePair<string, string>)this.FindAttribute(this.attributesToUpdate, applicationAttribute.Key));
                if (attrToUpdate.Key == null)
                {
                    this.lsvAttributes.Items.Add(this.CreateApplicationAttributeListViewItem(applicationAttribute));
                }
                else
                {
                    this.lsvAttributes.Items.Add(this.CreateApplicationAttributeListViewItem(attrToUpdate));
                }
            }
            //Add uncommitted sids 
            foreach (KeyValuePair<string, string> keyValue in this.attributesToAdd)
            {
                this.lsvAttributes.Items.Add(this.CreateApplicationAttributeListViewItem(keyValue));
            }
            ...
            if (!this.application.IAmManager)
                this.btnAdd.Enabled = this.btnRemove.Enabled = this.btnEdit.Enabled = this.btnOK.Enabled = this.btnApply.Enabled = false;
        ...
        private void frmApplicationAttributes_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.DialogResult == DialogResult.None)
                this.DialogResult = DialogResult.Cancel;
        }

        private IAzManAttribute<IAzManApplication> FindAttribute(IAzManAttribute<IAzManApplication>[] attributes, string key)
        {
            foreach (IAzManAttribute<IAzManApplication> a in attributes)
            {
                if (a.Key == key)
                    return a;
            }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                ...
                IAzManAttribute<IAzManApplication> applicationAttribute = this.FindAttribute(this.application.GetAttributes(), frm.key);
                if (keyValueToRemove.Key == null && keyValue1.Key == null && applicationAttribute == null)
                {
                    ...
                    ListViewItem lvi = this.CreateApplicationAttributeListViewItem(keyValueTaAdd);
        ...
        private void btnMembersRemove_Click(object sender, EventArgs e)
        {
            ...
            {
                if ((lvi.Tag as IAzManAttribute<IAzManApplication>) != null)
                {
                    IAzManAttribute<IAzManApplication> lviTag = (IAzManAttribute<IAzManApplication>)(lvi.Tag);
        ...
        private void btnApply_Click(object sender, EventArgs e)
        {
            ...
            {
                this.HourGlass(false);
                this.ShowError(ex.Message, "Update error");
            }
        }
        ...
        private void CommitChanges()
        {
            try
            {
                ...
                {
                    this.application.GetAttribute(Attribute.Key).Delete();
                }
                //Attribute To Add
                foreach (KeyValuePair<string, string> Attribute in this.attributesToAdd)
                {
                    this.application.CreateAttribute(Attribute.Key, Attribute.Value);
                }
                //Attribute To Update
                foreach (KeyValuePair<string, string> Attribute in this.attributesToUpdate)
                {
                    this.application.GetAttribute(Attribute.Key).Update(Attribute.Key, Attribute.Value);
                }
        ...
        private void lsvAttributes_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            if (this.lsvAttributes.CheckedItems.Count == 1 && e.CurrentValue == CheckState.Checked && e.NewValue == CheckState.Unchecked)
                ...
                this.btnRemove.Enabled = true && this.application.IAmManager;
        }
        ...
        {
            frmAttributeProperties frm = new frmAttributeProperties();
            if (this.lsvAttributes.SelectedItems[0].Tag as IAzManAttribute<IAzManApplication> != null)
            {
                IAzManAttribute<IAzManApplication> attr = this.lsvAttributes.SelectedItems[0].Tag as IAzManAttribute<IAzManApplication>;
                frm.key = attr.Key;
        ...
        {
            this.btnEdit.Enabled = (this.lsvAttributes.SelectedItems.Count == 1) && this.application.IAmManager;
        }

        private void lsvAttributes_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            this.btnRemove.Enabled = this.lsvAttributes.SelectedItems.Count > 0 && this.application.IAmManager;
            this.btnEdit.Enabled = this.lsvAttributes.SelectedItems.Count == 1 && this.application.IAmManager;
        }
